        ...
        <main class="relative flex flex-col h-screen py-10 px-16 gap-8 items-center justify-between">
            ...
            <div class="flex flex-col w-full max-w-[616px] items-center rounded-[64px] p-[52px] gap-8 bg-white">
                ...
                <div class="flex flex-col gap-6 w-full">
                    <div class="flex items-center rounded-3xl border border-tutor-border py-6 pl-8 pr-3 gap-4 bg-tutor-bg-grey">
                        <img src="{{asset('assets/images/icons/receipt-grey.svg') }}" class="w-6 flex shrink-0" alt="icon">
                        <div class="flex h-6 border-[1.5px] border-tutor-border"></div>
                        <p class="font-semibold text-lg leading-[22px] text-tutor-grey">Booking Code:</p>
                        <p class="font-bold text-xl leading-[25px]">
                            {{ $productSubscription->booking_trx_id }}
                        </p>
                    </div>
                    <p class="font-['Poppins'] font-medium leading-[25px] text-tutor-grey text-center">Pesanan Anda berhasil dibuat Kami sedang melakukan proses verifikasi, mohon menunggu beberapa menit 🎉 .</p>
                </div>
                <a href="{{ route('front.check_booking') }}" class="flex items-center rounded-full h-[60px] px-9 w-full gap-[6px] bg-tutor-orange justify-center">
